...
public interface CateRepository extends SmRepository<Cate, Integer> {
    @Override
    @Insert("INSERT INTO cate (cate_id, cate_name) VALUES (#{cateId},#{cateName})")
    ...
    @Override
    @Delete("DELETE FROM cate WHERE cate_id=#{integer}")
    ...
    @Override
    @Select("SELECT * FROM cate WHERE cate_id=#{integer}")
